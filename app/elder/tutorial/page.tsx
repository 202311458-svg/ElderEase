...
let globalPdfUserName = "User"; // This will hold the name for the PDF
 ...
 useEffect(() => {
  ...
  const unsubscribe = onAuthStateChanged(auth, async (user) => {
    ...
    if (snap.exists()) {
      ...
      console.log("Firestore userName:", data.fullName);

      setUserName(data.fullName); // 🔥 FORCE IT
    } else {
      setUserName("User");
    }
  });
...
const generatePDF = async (
  ...
  try {
    ...
    const categoryLabel =
      option === "all"
        ? "All Platforms"
        : platformData[option as keyof typeof platformData]?.name || option;

    // --- 1. HEADER ---
    doc.setFontSize(20);
    doc.setTextColor(0, 0, 0);
    doc.text("ElderEase: Learning Progress Report", 14, 20);

    doc.setFontSize(11);
    doc.setTextColor(80, 80, 80);
    doc.text(`Category: ${categoryLabel} | Generated: ${date}`, 14, 28);

    // --- 2. TABLE ---
    autoTable(doc, {
      startY: 35,
      head: [["#", "Platform", "Tutorial Title", "Duration", "Status"]],
      body: rows.map((row: any, i: number) => [
        String(i + 1),
        String(
          platformData[row.platform as keyof typeof platformData]?.name ||
            row.platform
        ),
        String(row.title),
        `${row.durationMin} min`,
    ...
    // --- 3. FOOTER (Printed by Firestore Name ONLY) ---
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // ✅ ONLY Firestore name
      doc.text(`Printed by: ${userName}`, 14, pageHeight - 10);
    ...
    if (doPrint) {
      ...
      doc.save(
        `Report-${userName.replace(/\s+/g, "-")}-${Date.now()}.pdf`
      );
    }
  } catch (error) {
